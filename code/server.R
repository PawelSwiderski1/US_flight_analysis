...
server <- function(input, output, session) {
    ...
    timeDelayPlot <- df %>%
      ...
      geom_text(aes(label =  paste(
        as.character(round(as.numeric(
          substr(lubridate::dminutes(avg_delay), 1, 5)
        )) %/% 60),
        "m",
        as.character(round(as.numeric(
          substr(lubridate::dminutes(avg_delay), 1, 5)
  ...
  output$weekday_delay_comb = renderPlot({
    weekday_delay_comb
  }, bg = "transparent")
  output$month_delay_comb = renderPlot({
    monthDelayPlot
  }, bg = "transparent")
  ...
  output$cancelledPlot = renderPlot({cancelledPlot}, bg="transparent")
  
  output$stateDelayPlot = renderPlot({stateDelayPlot}, bg="transparent")
  output$routesDelayPlot = renderPlot({routesDelayPlot}, bg="transparent")
